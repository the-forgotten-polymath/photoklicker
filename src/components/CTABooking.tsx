...
export default function CTABooking() {
  ...
  const handleWhatsAppRedirect = (e: React.FormEvent) => {
    e.preventDefault();
    const phoneNumber = "919359995662"; // Real number from photoklicker data
    const text = `Hi, I'm ${formData.name}. I'd like to book a PhotoKlicker photography session for a ${formData.event} on ${formData.date}. \n\nMessage: ${formData.message}`;
    ...
    <section id="contact" className="relative py-20 md:py-32 px-4 md:px-6 flex items-center justify-center overflow-hidden">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="font-body text-white/70 max-w-[500px] mx-auto leading-relaxed">
            Book a free consultation today. Let’s freeze time, tell your story, and craft beautiful masterpieces of your most precious milestones.
          </p>
        </div>
        ...
        <form onSubmit={handleWhatsAppRedirect} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <option className="text-black" value="Ring Ceremony">Ring Ceremony</option>
              <option className="text-black" value="Maternity Shoot">Maternity Shoot</option>
              <option className="text-black" value="Baby Photography">Baby Photography</option>
          ...
          <div className="md:col-span-2 flex justify-center mt-4">
            <button 
              type="submit" 
              className="bg-[#25D366] hover:bg-[#128C7E] text-white px-8 py-4 rounded-full font-semibold flex items-center gap-3 transition-all hover:scale-105 shadow-lg cursor-pointer"
            >
